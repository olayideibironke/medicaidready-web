...
async function ensureProviderRow(sb: ReturnType<typeof supabaseAdmin>, providerId: string) {
  const { data: existing, error: exErr } = await sb
    .from("providers")
    .select("id, checklist, updated_at")
    .eq("id", providerId)
    .maybeSingle();
  ...
  if (existing) {
    const checklist = Array.isArray((existing as any).checklist) ? ((existing as any).checklist as ChecklistItem[]) : [];
    return {
      exists: true,
      checklist,
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method === "GET") {
    const route = req.url ? req.url.split("?")[0] : `/api/providers/${providerId}/checklist`;
    const method = "GET";
  ...
  if (req.method === "PUT" || req.method === "PATCH") {
    ...
    const body = req.body ?? {};
    const itemsInput = Array.isArray(body.items) ? body.items : body.key ? [body] : [];
