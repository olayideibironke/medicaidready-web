...
async function ensureProviderRow(sb: ReturnType<typeof supabaseAdmin>, providerId: string) {
  const { data: existing, error: exErr } = await sb
    .from("providers")
    .select("id, created_at, updated_at, onboard, checklist")
    .eq("id", providerId)
    .maybeSingle();
  ...
  // Preserve legacy behavior: GET snapshot auto-creates a record behind it
  const t = nowIso();
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const route = req.url ? req.url.split("?")[0] : `/api/providers/${providerId}/snapshot`;
  const method = "GET";
