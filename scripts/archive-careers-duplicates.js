...
function pickKeeper(group) {
  ...
  jobs.sort((a, b) => {
    const aImported = a.source_type === "imported" ? 1 : 0;
    const bImported = b.source_type === "imported" ? 1 : 0;
...
async function main() {
  ...
  if (!reportPath) {
    ...
    console.error("Usage:");
    console.error(
      "node scripts/archive-careers-duplicates.js reports/careers-audit-file.json --dry-run"
    );
    console.error(
      "node scripts/archive-careers-duplicates.js reports/careers-audit-file.json"
    );
    process.exit(1);
  }
  ...
  const groups =
    report &&
    report.duplicates &&
    Array.isArray(report.duplicates.title_company_location)
      ? report.duplicates.title_company_location
      : [];
  ...
  console.log(`Duplicate title+company+location groups: ${groups.length}`);
  console.log(`Unique duplicate jobs to archive: ${ids.length}`);
  console.log("");

  for (const decision of decisions) {
    console.log(`Group: ${decision.key}`);
  ...
  console.log(`Archived ${data ? data.length : 0} duplicate jobs.`);
  console.log("Done.");
}
